  ...
  R6::R6Class("DataReporter",
              ...
              public = list(
                ...
                get_results = function() {
                  testdat_results(self$results$as_list())
                },
...
context_data <- function(data) {
  ...
  if ("set_data" %in% names(reporter)) {
    reporter$set_data(data)
  } else {
    stop("Tried to set data context on non-data reporter", call. = FALSE)
  }
}
...
#' Create a `testdat_results` object from the test results
#' as stored in the DataReporter results field.
#'
#' @param results a list as stored in DataReporter
#' @return its list argument as a `testdat_results` object
#' @seealso DataReporter
#' @keywords internal
testdat_results <- function(results) {
  stopifnot(is.list(results))
  structure(results, class = "testdat_results")
}
...
all_passed <- function(res) {
  ...
  df <- as.data.frame.testdat_results(res)
...
#' @export
as.data.frame.testdat_results <- function(x, ...) {
  if (length(x) == 0) {
    return(data.frame())
  }
...
sumarize_one_test_results <- function(test) {
  ...
  if (nb_tests > 0) {
    ...
    error <- testthat:::expectation_error(last_test)
    if (error) {
      test_results <- test_results[-nb_tests]
      nb_tests <- length(test_results)
    }

    nb_passed <- sum(vapply(test_results, testthat:::expectation_success, logical(1)))
    nb_skipped <- sum(vapply(test_results, testthat:::expectation_skip, logical(1)))
    nb_failed <- sum(vapply(test_results, testthat:::expectation_failure, logical(1)))
    nb_warning <- sum(vapply(test_results, testthat:::expectation_warning, logical(1)))
  }
...
#' @export
print.testdat_results <- function(x, ...) {
  print(as.data.frame(x))
}

# summary.testdat_results <- function(x, ...) {
#   lapply(x, function(d) {
#     cat
#   }
# }
