...
In general, our approach to data testing is variable-centric.
...
* `vars` require a variable set specified by dplyr's `vars()` function.
...
test_that("iris range checks", {
  expect_range(Petal.Width, 0, 1)
})

test_that("iris range checks filtered", {
  # Test passes for setosa rows
  expect_range(Petal.Width, 0, 1, flt = Species == "setosa")
  # Failures will provide the filter
  expect_range(Petal.Width, 0, 0.5, flt = Species == "setosa")
})
```
...
:   Man page: [generic-expectations](../help/generic-expectations)
    ...
    the [chk-generic](../help/chk-generic) man page.
