...
[R packages](https://r-pkgs.org/tests.html).
...
expectations perform a check on one or more variables in a given dataset.
...
in `data`. Only rows where the condition evaluates to `TRUE` are included in the
test.
...
By default, `set_testdata()` stores a reference to the provided dataset rather
than the data itself, so changes made to the dataset will be reflected in the
test results.
...
to a testing approach, using a simple sample processing exercise.
...
* The global dataset must be reset after changes are made, either with
`set_testdata()` or `with_testdata()`.
...
vignette. See [R packages](https://r-pkgs.org/tests.html) for a brief
